...
2. **GOLDEN TEMPLATE**: Every lookup entry must follow this format:
   - Header (# Name)
   - Location (*Location: path*)
   - Responsibility (**Responsibility**)
   - Signatures (### signature)
3. **STRICT SIGNATURES**: Every method must have a strict Python signature. Vague descriptions are violations.
4. **LOCATION DEFINITION**: Every entry must have a single, definitive file path.
5. **NO VIBE-CODING**: Do not crawl raw source code if a lookup entry exists.
