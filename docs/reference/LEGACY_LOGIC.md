...
*   **Status**: ✅ **Implemented** (in `MetadataService`).
...
## 3. 🔡 String Sanitization Rules

The application enforces strict character replacement to ensure file system compatibility and standard casing.

### 3.1. The "Anti-Cro" Character Map (⚠️ DEPRECATED / FALLBACK ONLY)
*   **Legacy Behavior**: Aggressively replaced Croatian characters to ensure ASCII compatibility (`đ` -> `dj`, `š` -> `s`, etc.).
