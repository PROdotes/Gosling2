...
class FilingService:
    ...
    def _load_rules(self) -> dict:
        ...
        if not self._rules_path.exists():
            logger.warning(f"[FilingService] Rules file not found at {self._rules_path}. Using fallback.")
            return {"routing_rules": [], "default_rule": "{genre}/{year}/{artist} - {title}"}
    ...
    def evaluate_routing(self, song: Song) -> Path:
        ...
        genre_tags = [t for t in song.tags if t.category and t.category.lower() == "genre"]
        ...
        if not primary_genre_tag:
            raise ValueError(f"Song {song.id} is missing a Primary Genre (required for routing)")
        ...
        if not performers:
            raise ValueError(f"Song {song.id} is missing Performer credits (required for routing)")

        artist_str = ", ".join(performers)
        
        if not song.media_name:
            raise ValueError(f"Song {song.id} is missing a Title/MediaName (required for routing)")
            
        if not song.year:
             raise ValueError(f"Song {song.id} is missing a Recording Year (required for routing)")
        ...
        if not rule_path_template:
            logger.warning(f"[FilingService] <- evaluate_routing() FAILURE: No rule matches genre '{genre_name}'")
            raise ValueError(f"No filing rule exists for genre '{genre_name}'. Add it to rules.json.")
    ...
    def copy_to_library(self, song: Song, library_root: Path) -> Path:
        ...
        if target_absolute.exists():
            raise FileExistsError(f"Target path already exists in library: {target_absolute}")
