...
def check_conflict(target_path: str) -> bool:
    """
...
def _resolve_pattern(pattern: str, song, overrides: dict = None) -> str:
    ...
    # Clean search payload (e.g. "ABBA ::: Ella" -> "ABBA")
    if isinstance(artist, str) and " ::: " in artist:
        artist = artist.split(" ::: ")[0]

    artist_clean = _sanitize(artist or "Unknown Artist")
    title_clean = _sanitize(song.title or "Unknown Title")
    ...
    data = {
        ...
        "Artist": artist_clean,
        "Title": title_clean,
        "Album": _sanitize(album or "Unknown Album"),
        "Composers": _sanitize(composer_str),
        "Genre": _sanitize(genre_name),
        "Year": _sanitize(str(song.year) if song.year else "0000"),
        "BPM": _sanitize(str(song.bpm) if hasattr(song, 'bpm') and song.bpm else "0"),
...
class RenamingService:
    ...
    def calculate_target_path(self, song) -> str:
        ...
        if hasattr(song, 'source_id') and song.source_id:
            try:
                ...
                logger.debug(f"Failed to import tag services or registry: {e}")
            else:
                try:
                    ts = TagService()
                    db_tags = ts.get_tags_for_source(song.source_id) or []
                    for t in db_tags:
                        cat = getattr(t, "category", None)
                        if cat and ID3Registry.get_id3_frame(cat) == "TCON":
                            tag_name = getattr(t, "tag_name", None)
                            if tag_name:
                                song_genres.append(tag_name.lower().strip())
                except Exception as e:
                    from ...core import logger
                    logger.debug(f"Failed to retrieve tags for source {getattr(song, 'source_id', None)}: {e}")
        ...
        rel_path = _resolve_pattern(pattern, song, overrides={'genre': primary_genre})
    ...
    def rename_song(self, song, target_path: str = None) -> tuple[bool, str]:
        ...
        if check_conflict(target_path):
            return False, f"Destination already exists: {os.path.basename(target_path)}"
            
        # Extension check to ensure we don't do something weird
