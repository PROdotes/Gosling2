...
async def get_history(table: str, record_id: int) -> List[Dict[str, Any]]:
    ...
    """
    logger.debug(f"[AuditRouter] get_history({table}, {record_id})")
    ...
    try:
        ...
        if not history:
            logger.debug(f"[AuditRouter] history NOT_FOUND for {table}:{record_id}")
            # Returning empty list is fine for history
            return []
        return history
    except Exception as e:
        logger.error(
            f"[AuditRouter] ERROR: history fetch failure for {table}:{record_id} - {str(e)}"
        )
