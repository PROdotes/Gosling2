...
class TestSearchSlimDeepFields:
    ...
    def test_search_by_album_title(self, populated_db):
        ...
        assert len(rows) == 1, f"Expected 1 match for album 'Nevermind', got {len(rows)}"
        assert rows[0]["SourceID"] == 1, f"Expected SourceID=1, got {rows[0]['SourceID']}"
    ...
    def test_search_by_isrc(self, populated_db):
        ...
        assert rows[0]["SourceID"] == 7, f"Expected SourceID=7, got {rows[0]['SourceID']}"
        assert rows[0]["ISRC"] == "ISRC123", f"Expected 'ISRC123', got '{rows[0]['ISRC']}'"
    ...
    def test_search_by_role_is_ignored(self, populated_db):
        ...
        assert len(rows) == 0, f"Expected 0 matches for role 'Composer', got {len(rows)}"
