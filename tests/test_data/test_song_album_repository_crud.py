...
class TestAddAlbum:
    def test_add_album_link(self, populated_db):
        ...
        assert albums[0].album_id == 100, f"Expected AlbumID=100, got {albums[0].album_id}"
        assert albums[0].track_number == 5, f"Expected TrackNumber=5, got {albums[0].track_number}"
        assert albums[0].disc_number == 1, f"Expected DiscNumber=1, got {albums[0].disc_number}"
        assert albums[0].source_id == 3, f"Expected SourceID=3, got {albums[0].source_id}"
    ...
    def test_add_album_idempotent_on_duplicate(self, populated_db):
        ...
        albums = repo.get_albums_for_songs([3])
        assert len(albums) == 1, f"Expected 1 album link (idempotent), got {len(albums)}"
    ...
    def test_add_album_does_not_affect_other_songs(self, populated_db):
        ...
        assert len(after) == len(before), f"Song 1 album count should not change: expected {len(before)}, got {len(after)}"
        assert after[0].album_id == 100, f"Song 1 should still link to Nevermind (100), got {after[0].album_id}"
    ...
    def test_add_album_keeps_album_record(self, populated_db):
        ...
        with repo._get_connection() as conn:
            ...
            row = conn.execute("SELECT AlbumTitle FROM Albums WHERE AlbumID = 100").fetchone()
            assert row["AlbumTitle"] == "Nevermind", f"Expected album title 'Nevermind' unchanged, got '{row['AlbumTitle']}'"
...
class TestRemoveAlbum:
    def test_remove_album_deletes_link(self, populated_db):
        ...
        assert len(albums) == 0, f"Expected 0 albums on Song 1 after remove, got {len(albums)}"

        # Album record persists
        with repo._get_connection() as conn:
            conn.row_factory = sqlite3.Row
            row = conn.execute("SELECT AlbumID FROM Albums WHERE AlbumID = 100").fetchone()
            assert row is not None, "Expected Album record (ID=100) to persist after link removal"
    ...
    def test_remove_album_does_not_affect_other_songs(self, populated_db):
        ...
        assert len(albums_song2) == 1, f"Expected Song 2 to still have 1 album, got {len(albums_song2)}"
        assert albums_song2[0].album_id == 200, f"Expected AlbumID=200, got {albums_song2[0].album_id}"
...
class TestUpdateTrackInfo:
    def test_update_track_number(self, populated_db):
        ...
        assert albums[0].track_number == 3, f"Expected TrackNumber=3, got {albums[0].track_number}"
        assert albums[0].disc_number == 1, f"Expected DiscNumber=1 unchanged, got {albums[0].disc_number}"
    ...
    def test_update_disc_number(self, populated_db):
        ...
        assert albums[0].disc_number == 2, f"Expected DiscNumber=2, got {albums[0].disc_number}"
        assert albums[0].track_number == 11, f"Expected TrackNumber=11 unchanged, got {albums[0].track_number}"
