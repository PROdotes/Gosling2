...
class TestGetTagsForSongs:
    def test_single_song_multiple_tags_returns_all(self, populated_db):
        ...
        assert tag_names == ["Energetic", "English", "Grunge"], f"Expected ['Energetic', 'English', 'Grunge'], got {tag_names}"
        ...
        assert grunge.is_primary is False, f"Expected is_primary=False (Song 1 tags all have IsPrimary=0), got {grunge.is_primary}"

        energetic = tag_map["Energetic"]
        assert energetic.id == 2, f"Expected id=2, got {energetic.id}"
        assert energetic.name == "Energetic", f"Expected 'Energetic', got '{energetic.name}'"
        assert energetic.category == "Mood", f"Expected 'Mood', got '{energetic.category}'"
        assert energetic.is_primary is False, f"Expected is_primary=False, got {energetic.is_primary}"
        ...
        assert english.category == "Jezik", f"Expected 'Jezik', got '{english.category}'"
        assert english.is_primary is False, f"Expected is_primary=False, got {english.is_primary}"
    ...
    def test_multiple_songs_returns_grouped_correctly(self, populated_db):
        ...
        # Song 1: 3 tags
        assert len(tags_by_song[1]) == 3, f"Expected 3 tags for Song 1, got {len(tags_by_song[1])}"
        song1_names = sorted([t.name for t in tags_by_song[1]])
        assert song1_names == ["Energetic", "English", "Grunge"], f"Expected ['Energetic', 'English', 'Grunge'], got {song1_names}"

        # Song 2: 1 tag
        assert len(tags_by_song[2]) == 1, f"Expected 1 tag for Song 2, got {len(tags_by_song[2])}"
        assert tags_by_song[2][0].name == "90s", f"Expected '90s', got '{tags_by_song[2][0].name}'"
        assert tags_by_song[2][0].category == "Era", f"Expected 'Era', got '{tags_by_song[2][0].category}'"

        # Song 4: 1 tag
        assert len(tags_by_song[4]) == 1, f"Expected 1 tag for Song 4, got {len(tags_by_song[4])}"
        assert tags_by_song[4][0].name == "Electronic", f"Expected 'Electronic', got '{tags_by_song[4][0].name}'"
        assert tags_by_song[4][0].category == "Style", f"Expected 'Style', got '{tags_by_song[4][0].category}'"
    ...
    def test_empty_list_returns_empty_results(self, populated_db):
        ...
        assert len(result) == 0, f"Expected 0 results for empty input, got {len(result)}"
    ...
    def test_nonexistent_song_id_returns_empty(self, populated_db):
        ...
        assert len(result) == 0, f"Expected 0 tags for nonexistent song, got {len(result)}"
...
class TestGetAll:
    def test_returns_all_tags_sorted_by_name(self, populated_db):
        ...
        assert tag.is_primary is False, f"Expected is_primary=False (not in row), got {tag.is_primary}"
...
class TestSearch:
    def test_exact_match_returns_single_result(self, populated_db):
        ...
        assert tag.is_primary is False, f"Expected is_primary=False, got {tag.is_primary}"
    ...
    def test_partial_match_returns_matching_tags(self, populated_db):
        ...
        assert len(result) >= 4, f"Expected at least 4 matches for 'e', got {len(result)}"
    ...
    def test_case_insensitive_search(self, populated_db):
        ...
        assert len(result) == 1, f"Expected 1 result (case-insensitive), got {len(result)}"
    ...
    def test_no_matches_returns_empty_list(self, populated_db):
        ...
        assert len(result) == 0, f"Expected 0 results for nonexistent term, got {len(result)}"
    ...
    def test_results_sorted_by_name(self, populated_db):
        ...
        assert names == sorted_names, f"Expected sorted names {sorted_names}, got {names}"
...
class TestGetById:
    def test_existing_tag_returns_complete_object(self, populated_db):
        ...
        assert tag.name == "Grunge", f"Expected 'Grunge', got '{tag.name}'"
        assert tag.category == "Genre", f"Expected 'Genre', got '{tag.category}'"
        assert tag.is_primary is False, f"Expected is_primary=False (not in row keys), got {tag.is_primary}"
...
class TestGetSongIdsByTag:
    ...
    def test_tag_with_multiple_songs_returns_all(self, populated_db):
        ...
        assert len(song_ids) == 2, f"Expected 2 songs for Tag 1 (Grunge), got {len(song_ids)}"
    ...
    def test_tag_with_no_songs_returns_empty(self, populated_db):
        ...
        assert len(song_ids) == 0, f"Expected 0 songs for orphan tag, got {len(song_ids)}"
    ...
    def test_nonexistent_tag_id_returns_empty(self, populated_db):
        ...
        assert len(song_ids) == 0, f"Expected 0 songs for nonexistent tag, got {len(song_ids)}"
...
class TestRowToTag:
    def test_all_fields_present(self, populated_db):
        ...
        assert tag.category == "Test Category", f"Expected 'Test Category', got '{tag.category}'"
        assert tag.is_primary is True, f"Expected is_primary=True (1→True), got {tag.is_primary}"
    ...
    def test_is_primary_false(self, populated_db):
        ...
        assert tag.is_primary is False, f"Expected is_primary=False (0→False), got {tag.is_primary}"
    ...
    def test_is_primary_missing_defaults_false(self, populated_db):
        ...
        assert tag.is_primary is False, f"Expected is_primary=False (missing key), got {tag.is_primary}"
