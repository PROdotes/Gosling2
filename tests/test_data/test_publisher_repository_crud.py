...
class TestAddSongPublisher:
    def test_add_existing_publisher_to_song(self, populated_db):
        ...
        assert result.id == 5, f"Expected PublisherID=5 (existing Sub Pop), got {result.id}"
        assert result.name == "Sub Pop", f"Expected name='Sub Pop', got '{result.name}'"

        # Verify no duplicate Publisher row
        with repo._get_connection() as conn:
            rows = conn.execute("SELECT PublisherID FROM Publishers WHERE PublisherName = 'Sub Pop'").fetchall()
            assert len(rows) == 1, f"Expected 1 Sub Pop row (no duplicate), got {len(rows)}"

        # Verify link created
        publishers = repo.get_publishers_for_songs([3])
        assert len(publishers) == 1, f"Expected 1 publisher on Song 3, got {len(publishers)}"
        assert publishers[0][1].id == 5, f"Expected PublisherID=5, got {publishers[0][1].id}"
    ...
    def test_add_new_publisher_creates_row(self, populated_db):
        ...
        assert result.name == "Interscope Records", f"Expected name='Interscope Records', got '{result.name}'"
        assert result.id is not None, f"Expected id to be set, got None"

        publishers = repo.get_publishers_for_songs([3])
        assert len(publishers) == 1, f"Expected 1 publisher on Song 3, got {len(publishers)}"
        assert publishers[0][1].name == "Interscope Records", f"Expected 'Interscope Records', got '{publishers[0][1].name}'"
    ...
    def test_add_publisher_idempotent_on_duplicate(self, populated_db):
        ...
        assert len(sub_pop) == 1, f"Expected 1 Sub Pop link (idempotent), got {len(sub_pop)}"
    ...
    def test_add_publisher_does_not_affect_other_songs(self, populated_db):
        ...
        assert len(after) == len(before), f"Song 1 publisher count should not change: expected {len(before)}, got {len(after)}"
...
class TestRemoveSongPublisher:
    def test_remove_publisher_deletes_link(self, populated_db):
        ...
        with repo._get_connection() as conn:
            repo.remove_song_publisher(1, 10, conn)  # Remove DGC Records (ID=10) from Song 1
            conn.commit()

        publishers = repo.get_publishers_for_songs([1])
        assert len(publishers) == 0, f"Expected 0 publishers on Song 1 after remove, got {len(publishers)}"

        # Publisher record persists
        pub = repo.get_by_id(10)
        assert pub is not None, "Expected Publisher record (ID=10) to persist after link removal"
    ...
    def test_remove_publisher_does_not_affect_other_songs(self, populated_db):
        ...
        assert len(publishers_song3) == 1, f"Expected Song 3 to still have 1 publisher, got {len(publishers_song3)}"
        assert publishers_song3[0][1].id == 10, f"Expected DGC Records on Song 3, got {publishers_song3[0][1].id}"
...
class TestUpdatePublisher:
    def test_update_publisher_name(self, populated_db):
        ...
        assert pub.name == "Sub Pop Records", f"Expected name='Sub Pop Records', got '{pub.name}'"
    ...
    def test_update_publisher_does_not_affect_other_publishers(self, populated_db):
        ...
        assert roswell.name == "Roswell Records", f"Expected 'Roswell Records' unchanged, got '{roswell.name}'"
