...
def _import_credits_bulk(db_path, song_id, credits, publishers):
    ...
    add_items = [
        AddCreditItem(type="credit", song_id=song_id, name=c.name, role=c.role, id=c.identity_id)
        for c in credits
    ] + [
        AddPublisherItem(type="publisher", song_id=song_id, name=p)
        for p in publishers
    ]
...
class TestCatalogBulkImport:
    def test_import_credits_bulk_success(self, catalog_service):
        ...
        assert nile_credit.role_name == "Guitar", f"Expected role 'Guitar', got {nile_credit.role_name}"

        pub_names = {p.name for p in song.publishers}
        assert "Sony/ATV" in pub_names, f"Expected 'Sony/ATV' in {pub_names}"
        assert "Universal Music" in pub_names, f"Expected 'Universal Music' in {pub_names}"
    ...
    def test_import_credits_bulk_idempotency(self, catalog_service):
        ...
        assert count_1 == count_2, f"Expected idempotent import, count changed from {count_1} to {count_2}"
    ...
    def test_import_credits_bulk_transactional_rollback(self, catalog_service):
        ...
        assert after_names == before_names, f"Expected unchanged state, got diff: {after_names ^ before_names}"
    ...
    def test_import_credits_bulk_with_resolved_identity(self, catalog_service):
        ...
        assert match.identity_id == 1, f"Expected identity 1 for 'The Drummer', got {match.identity_id}"
