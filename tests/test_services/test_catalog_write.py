...
class TestCatalogServiceIngestFile:
    ...
    def test_ingest_path_collision_returns_already_exists(self, populated_db, tmp_path):
        ...
        assert song.source_path == str(real_path), f"Expected {real_path}, got {song.source_path}"
        ...
        assert song.processing_status is None, f"Expected None (fixture default), got {song.processing_status}"
    ...
    def test_ingest_hash_collision_returns_already_exists(self, populated_db, test_mp3, monkeypatch):
        ...
        assert report["match_type"] == "HASH", f"Expected HASH, got {report['match_type']}"

        # Exhaustive song assertions
        song = report["song"]
        assert song.id == 1, f"Expected collision with Song 1, got {song.id}"
        assert song.title == "Smells Like Teen Spirit", f"Expected 'Smells Like Teen Spirit', got {song.title}"
        assert song.source_path == "/path/1", f"Expected '/path/1', got {song.source_path}"
        assert song.duration_s == 200, f"Expected 200s, got {song.duration_s}"
        assert song.audio_hash == "hash_1", f"Expected 'hash_1', got {song.audio_hash}"
        assert song.year == 1991, f"Expected 1991, got {song.year}"
        assert song.bpm is None, f"Expected None for bpm, got {song.bpm}"
    ...
    def test_ingest_unicode_metadata_handles_correctly(self, ingest_db, unicode_mp3):
        ...
        assert report["status"] == "INGESTED", f"Expected INGESTED, got {report['status']}"

        # Exhaustive song assertions
        song = report["song"]
        assert song.id is not None, "Expected DB-assigned ID"
        assert song.title == "日本語タイトル", f"Expected '日本語タイトル', got {song.title}"
        assert song.source_path == unicode_mp3, f"Expected {unicode_mp3}, got {song.source_path}"
        assert 2.0 < song.duration_s < 3.0, f"Expected ~2.27s, got {song.duration_s}"
        assert song.audio_hash is not None, "Expected audio hash to be calculated"
        assert song.year is None, f"Expected None for year, got {song.year}"
        assert song.bpm is None, f"Expected None for bpm, got {song.bpm}"
        assert song.isrc == "USCGJ2326543", f"Expected 'USCGJ2326543' (inherited from fixture file), got {song.isrc}"
        assert song.is_active is False, f"Expected default is_active=False, got {song.is_active}"
        assert song.processing_status is None, f"Expected default processing_status=None, got {song.processing_status}"
...
class TestCatalogServiceDeleteSong:
    ...
    def test_delete_cascades_work_correctly(self, populated_db):
        ...
        with service._song_repo.get_connection() as conn:
            ...
            albums = conn.execute("SELECT count(*) FROM SongAlbums WHERE SourceID = ?", (song_id,)).fetchone()[0]
        ...
        album = service.get_album(100)  # Album "Nevermind" (ID=100 per conftest.py line 111)
