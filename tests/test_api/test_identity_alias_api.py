...
class TestAddIdentityAliasApi:
    ...
    def test_add_new_alias_returns_200_and_body(self, api):
        resp = api.post("/api/v1/identities/1/aliases", json={"display_name": "D. Grohl"})
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}: {resp.text}"
        data = resp.json()
        assert "name_id" in data, f"Expected 'name_id' in response, got {data}"
        assert data["display_name"] == "D. Grohl", f"Expected 'D. Grohl', got {data['display_name']}"
    ...
    def test_add_alias_with_name_id_relinks(self, api):
        ...
        resp = api.post("/api/v1/identities/2/aliases", json={"display_name": "Grohlton", "name_id": 11})
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}: {resp.text}"
        data = resp.json()
        assert data["name_id"] == 11, f"Expected name_id=11, got {data['name_id']}"
        assert data["display_name"] == "Grohlton", f"Expected 'Grohlton', got {data['display_name']}"

    def test_add_alias_invalid_identity_returns_404(self, api):
        resp = api.post("/api/v1/identities/9999/aliases", json={"display_name": "Ghost"})
    ...
    def test_add_alias_steal_primary_with_siblings_returns_409(self, api):
        ...
        resp = api.post("/api/v1/identities/2/aliases", json={"display_name": "Dave Grohl", "name_id": 10})
...
class TestRemoveIdentityAliasApi:
    ...
    def test_remove_nonexistent_name_id_returns_204(self, api):
        ...
        assert resp.status_code == 204, f"Expected 204 (noop), got {resp.status_code}: {resp.text}"
