    ...
    def test_blank_role_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {
                    "add": [
                        {"type": "credit", "song_id": 1, "name": "Freddie", "role": ""}
                    ]
                }
            )
...
class TestUpdateSongItem:
    ...
    def test_null_media_name_allowed(self):
        ...
        MutationRequest.model_validate(
            {"update": [{"type": "song", "id": 1, "media_name": None}]}
        )

    def test_invalid_isrc_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"update": [{"type": "song", "id": 1, "isrc": "bad"}]}
            )

    def test_valid_isrc(self):
        MutationRequest.model_validate(
            {"update": [{"type": "song", "id": 1, "isrc": "GBAYE0000001"}]}
        )

    def test_zero_bpm_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"update": [{"type": "song", "id": 1, "bpm": 0}]}
            )

    def test_negative_bpm_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"update": [{"type": "song", "id": 1, "bpm": -1}]}
            )

    def test_year_out_of_range_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"update": [{"type": "song", "id": 1, "year": 99}]}
            )

    def test_year_valid(self):
        MutationRequest.model_validate(
            {"update": [{"type": "song", "id": 1, "year": 2024}]}
        )
...
class TestUpdateTagItems:
    def test_tag_entity_update(self):
        MutationRequest.model_validate(
            {"update": [{"type": "tag", "id": 5, "name": "Rock", "category": "Genre"}]}
        )

    def test_tag_entity_empty_name_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"update": [{"type": "tag", "id": 5, "name": ""}]}
            )
...
class TestUpdateAlbumItems:
    ...
    def test_album_entity_bad_year_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"update": [{"type": "album", "id": 3, "release_year": 75}]}
            )
...
class TestUpdatePublisherItem:
    def test_valid(self):
        MutationRequest.model_validate(
            {"update": [{"type": "publisher", "id": 9, "name": "EMI", "parent_id": 12}]}
        )

    def test_empty_name_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"update": [{"type": "publisher", "id": 9, "name": ""}]}
            )

    def test_null_name_allowed(self):
        MutationRequest.model_validate(
            {"update": [{"type": "publisher", "id": 9, "name": None}]}
        )
...
class TestUpdateCreditItem:
    def test_valid(self):
        MutationRequest.model_validate(
            {"update": [{"type": "credit", "id": 1, "display_name": "F. Mercury"}]}
        )

    def test_empty_display_name_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"update": [{"type": "credit", "id": 1, "display_name": ""}]}
            )
...
class TestRemoveItems:
    def test_remove_credit(self):
        MutationRequest.model_validate(
            {"remove": [{"type": "credit", "song_id": 1, "id": 42}]}
        )

    def test_remove_tag(self):
        MutationRequest.model_validate(
            {"remove": [{"type": "tag", "song_id": 1, "id": 7}]}
        )

    def test_remove_publisher(self):
        MutationRequest.model_validate(
            {"remove": [{"type": "publisher", "song_id": 1, "id": 3}]}
        )

    def test_remove_album(self):
        MutationRequest.model_validate(
            {"remove": [{"type": "album", "song_id": 1, "id": 5}]}
        )

    def test_missing_id_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"remove": [{"type": "credit", "song_id": 1}]}
            )
...
class TestMixedRequest:
    ...
    def test_unknown_type_422(self):
        with pytest.raises(ValidationError):
            MutationRequest.model_validate(
                {"add": [{"type": "unknown_entity", "song_id": 1}]}
            )
