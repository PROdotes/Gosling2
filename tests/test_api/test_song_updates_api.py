...
class TestUpdateSongScalars:
    def test_update_title_returns_full_song(self, api):
        ...
        assert data["media_name"] == "Renamed Title", f"Expected 'Renamed Title', got {data['media_name']}"
        assert data["title"] == "Renamed Title", f"Expected title='Renamed Title', got {data['title']}"
        assert data["source_path"] == "/path/1", f"Expected '/path/1', got {data['source_path']}"
        assert data["duration_s"] == 200.0, f"Expected 200.0, got {data['duration_s']}"
        assert data["audio_hash"] == "hash_1", f"Expected 'hash_1', got {data['audio_hash']}"
    ...
    def test_update_year_does_not_wipe_other_fields(self, api):
        ...
        assert data["media_name"] == "Hollow Song", f"Expected 'Hollow Song', got {data['media_name']}"
        assert data["bpm"] == 128, f"Expected bpm=128 (unchanged), got {data['bpm']}"
        assert data["isrc"] == "ISRC123", f"Expected isrc='ISRC123' (unchanged), got {data['isrc']}"
        assert data["source_path"] == "/path/7", f"Expected '/path/7', got {data['source_path']}"
    ...
    def test_update_bpm_does_not_wipe_other_fields(self, api):
        ...
        assert data["isrc"] == "ISRC123", f"Expected isrc='ISRC123' (unchanged), got {data['isrc']}"
        assert data["media_name"] == "Hollow Song", f"Expected 'Hollow Song' (unchanged), got {data['media_name']}"

    def test_update_isrc_does_not_wipe_other_fields(self, api):
        resp = api.patch("/api/v1/songs/1", json={"isrc": "USRC12345678"})
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}"
        data = resp.json()
        assert data["isrc"] == "USRC12345678", f"Expected 'USRC12345678', got {data['isrc']}"
        assert data["media_name"] == "Smells Like Teen Spirit", f"Expected title unchanged, got {data['media_name']}"
        assert data["year"] == 1991, f"Expected year=1991 (unchanged), got {data['year']}"
        assert data["audio_hash"] == "hash_1", f"Expected hash unchanged, got {data['audio_hash']}"
    ...
    def test_partial_update_persistence_exhaustive(self, api):
        ...
        data = resp.json()
        assert data["bpm"] == 140, f"Expected bpm=140, got {data['bpm']}"
        assert data["year"] == 2024, f"Expected year=2024 (unchanged), got {data['year']}"
        assert data["media_name"] == "Hollow Song", f"Expected title unchanged, got {data['media_name']}"
...
class TestSongCredits:
    def test_add_credit_returns_full_song_credit(self, api):
        ...
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}"
        data = resp.json()
        assert data["display_name"] == "Dave Grohl", f"Expected 'Dave Grohl', got {data['display_name']}"
        assert data["role_name"] == "Performer", f"Expected 'Performer', got {data['role_name']}"
...
class TestSongAlbums:
    def test_link_existing_album_returns_full_song_album(self, api):
        ...
        assert data["album_title"] == "Nevermind", f"Expected 'Nevermind', got {data['album_title']}"
        assert data["track_number"] == 5, f"Expected track_number=5, got {data['track_number']}"
        assert data["release_year"] == 1991, f"Expected release_year=1991, got {data['release_year']}"
    ...
    def test_create_and_link_album_returns_full_song_album(self, api):
        ...
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}"
        data = resp.json()
        assert data["album_title"] == "Brand New Album", f"Expected 'Brand New Album', got {data['album_title']}"
        assert data["release_year"] == 2024, f"Expected 2024, got {data['release_year']}"
    ...
    def test_update_album_link_success(self, api):
        ...
        assert link["track_number"] == 3, f"Expected track_number=3, got {link['track_number']}"
        assert link["disc_number"] == 2, f"Expected disc_number=2, got {link['disc_number']}"

    def test_update_album_link_partial_preserves_other_field(self, api):
        # Setup: multi-disc state
        api.patch("/api/v1/songs/2/albums/200", json={"track_number": 4, "disc_number": 2})
        ...
        link = next((a for a in song["albums"] if a["album_id"] == 200), None)
        assert link is not None, "Album link 200 not found on song 2"
        assert link["track_number"] == 5, f"Expected track_number=5, got {link['track_number']}"
        assert link["disc_number"] == 2, f"Expected disc_number=2 (unchanged), got {link['disc_number']}"
...
class TestAlbumUpdates:
    def test_update_album_title_returns_full_album(self, api):
        ...
        assert data["title"] == "Nevermind (Remaster)", f"Expected 'Nevermind (Remaster)', got {data['title']}"
        assert data["release_year"] == 1991, f"Expected release_year=1991 (unchanged), got {data['release_year']}"
    ...
    def test_update_album_year_does_not_wipe_title(self, api):
        ...
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}"
        data = resp.json()
        assert data["release_year"] == 1998, f"Expected 1998, got {data['release_year']}"
    ...
    def test_add_album_credit_success_and_visible(self, api):
        ...
        assert alb.status_code == 200, f"Expected 200 on GET album, got {alb.status_code}"
        data = alb.json()
...
class TestSongTags:
    ...
    def test_add_tag_does_not_wipe_existing_song_data(self, api):
        resp = api.post("/api/v1/songs/7/tags", json={"tag_name": "Ambient", "category": "Genre"})
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}"

        song = api.get("/api/v1/songs/7").json()
        assert song["bpm"] == 128, f"Expected bpm=128 (unchanged), got {song['bpm']}"
        assert song["isrc"] == "ISRC123", f"Expected isrc='ISRC123' (unchanged), got {song['isrc']}"
        assert song["media_name"] == "Hollow Song", f"Expected title unchanged, got {song['media_name']}"
    ...
    def test_remove_tag_success(self, api):
        ...
        assert 1 not in tag_ids, f"Tag 1 (Grunge) should be removed but still present: {tag_ids}"
...
class TestSongPublishers:
    def test_add_publisher_returns_full_publisher(self, api):
        ...
        assert data["id"] is not None, "Expected id to be set"
        assert "parent_id" in data, "Response missing 'parent_id'"
    ...
    def test_update_publisher_success_and_reflected_on_song(self, api):
        ...
        resp = api.patch("/api/v1/publishers/10", json={"publisher_name": "DGC (Universal)"})
        assert resp.status_code == 204, f"Expected 204, got {resp.status_code}"
