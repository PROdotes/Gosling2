...
class TestGetAllTags:
    ...
    def test_all_tags_present(self, api):
        ...
        for expected_id in [1, 2, 3, 4, 5, 6]:
            assert expected_id in ids, f"Expected tag_id={expected_id} in list, got {ids}"
...
class TestSearchTags:
    def test_search_returns_matching_tags(self, api):
        ...
        assert data[0]["category"] == "Genre", f"Expected 'Genre', got {data[0]['category']}"
    ...
    def test_search_excludes_non_matching(self, api):
        ...
        for excluded in [2, 3, 4, 5, 6]:
            assert excluded not in ids, f"Tag {excluded} should not match 'Grunge', got {ids}"
...
class TestGetTagCategories:
    def test_returns_distinct_categories(self, api):
        ...
        for expected in ["Genre", "Mood", "Era", "Style", "Jezik"]:
            assert expected in data, f"Expected category '{expected}' in list, got {data}"
...
class TestGetAllPublishers:
    ...
    def test_response_shape_matches_publisher_view(self, api):
        ...
        assert pub["name"] == "DGC Records", f"Expected name='DGC Records', got {pub['name']}"
        assert pub["parent_name"] == "Universal Music Group", f"Expected parent_name='Universal Music Group', got {pub['parent_name']}"
    ...
    def test_top_level_publisher_has_null_parent(self, api):
        ...
        assert umg["parent_name"] is None, f"Expected parent_name=None for UMG, got {umg['parent_name']}"
    ...
    def test_all_publishers_present(self, api):
        ...
        for expected_id in [1, 2, 3, 4, 5, 10]:
            assert expected_id in ids, f"Expected pub_id={expected_id} in list, got {ids}"
...
class TestSearchPublishers:
    def test_search_returns_matching_publishers(self, api):
        ...
        assert data[0]["name"] == "Sub Pop", f"Expected 'Sub Pop', got {data[0]['name']}"
    ...
    def test_search_excludes_non_matching(self, api):
        ...
        for excluded in [1, 2, 3, 4, 10]:
            assert excluded not in ids, f"Publisher {excluded} should not match 'Sub Pop', got {ids}"
...
class TestGetPublisherById:
    def test_existing_publisher_returns_200_with_full_shape(self, api):
        ...
        assert data["name"] == "DGC Records", f"Expected name='DGC Records', got {data['name']}"
        assert data["parent_name"] == "Universal Music Group", f"Expected parent_name='Universal Music Group', got {data['parent_name']}"
        assert isinstance(data["sub_publishers"], list), f"Expected sub_publishers list, got {type(data['sub_publishers'])}"
    ...
    def test_publisher_with_sub_publishers_includes_them(self, api):
        ...
        data = resp.json()
        assert data["id"] == 1, f"Expected id=1, got {data['id']}"
        assert data["name"] == "Universal Music Group", f"Expected 'Universal Music Group', got {data['name']}"
        assert data["parent_name"] is None, f"Expected parent_name=None, got {data['parent_name']}"
        sub_ids = [s["id"] for s in data["sub_publishers"]]
        assert 2 in sub_ids, f"Expected Island Records (id=2) in sub_publishers, got {sub_ids}"
        assert 10 in sub_ids, f"Expected DGC Records (id=10) in sub_publishers, got {sub_ids}"
    ...
    def test_publisher_no_sub_publishers_returns_empty_list(self, api):
        ...
        data = resp.json()
        assert data["sub_publishers"] == [], f"Expected empty sub_publishers for Sub Pop, got {data['sub_publishers']}"
