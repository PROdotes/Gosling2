...
def get_documented_map(lookup_dir: str):
    ...
    """
    doc_map = {}
    md_files = glob.glob(os.path.join(lookup_dir, "js_*.md")) + [
        os.path.join(lookup_dir, "dashboard_ui.md")
    ]
...
def get_exports_to_verify(file_path: str):
    ...
    matches = re.finditer(
        r"export\s+(?:async\s+)?(?:function|class|const)\s+([a-zA-Z0-9_]+)", content
    )
    for m in matches:
        ...
        for js_file in js_files:
            ...
            for loc_path, sigs in doc_map.items():
                ...
                if abs_path_norm == loc_path_norm or (
                    os.path.isdir(loc_path)
                    and abs_path_norm.startswith(loc_path_norm.rstrip("/") + "/")
                ):
                    is_covered = True
